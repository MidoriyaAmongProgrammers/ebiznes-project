...
class ShipmentController @Inject()(shipmentRepository: ShipmentRepository, cc: MessagesControllerComponents) extends MessagesAbstractController(cc) {
  ...
  def updateShipment(id: Int): Action[AnyContent] = Action.async { implicit request: MessagesRequest[AnyContent] =>
    ...
    shipment.map(shipment => {
      val shipmentForm = updateShipmentForm.fill(
        UpdateShipmentForm(shipment.id, shipment.date, shipment.address, shipment.firstName, shipment.lastName, shipment.provider))
  ...
  def updateShipmentHandle = Action.async { implicit request =>
    updateShipmentForm.bindFromRequest.fold(
      ...
      shipment => {
        shipmentRepository.update(shipment.id,
          Shipment(shipment.id, shipment.date, shipment.address, shipment.firstName, shipment.lastName, shipment.provider)).map { _ =>
  ...
  def createShipmentHandle = Action.async { implicit request =>
    ...
    shipmentForm.bindFromRequest.fold(
      ...
      shipment => {
        shipmentRepository.create(shipment.date, shipment.address, shipment.firstName, shipment.lastName, shipment.provider).map { _ =>
...
case class CreateShipmentForm(date: String, address: Int, firstName: String, lastName: String, provider: String)
case class UpdateShipmentForm(id: Int, date: String, address: Int, firstName: String, lastName: String, provider: String)