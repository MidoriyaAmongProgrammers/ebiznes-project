...
class ProductController @Inject()(productRepository: ProductRepository,
  ...
  def updateProduct(id: Int): Action[AnyContent] = Action.async { implicit request: MessagesRequest[AnyContent] =>
    ...
    product.map(product => {
      val prodForm = updateProductForm.fill(UpdateProductForm(product.id, product.name, product.price, product.description,product.subCategory, product.img))
    ...
    updateProductForm.bindFromRequest.fold(
      ...
      product => {
        productRepository.update(product.id,
          Product(product.id, product.name, product.price, product.description, product.subCategory, product.img)).map { _ =>
  ...
  def createProductHandle = Action.async { implicit request =>
    ...
    productForm.bindFromRequest.fold(
      ...
      product => {
        productRepository.create(product.name, product.price, product.description, product.subCategory, product.img).map { _ =>
...
case class CreateProductForm(name: String, price: Float, description: String, subCategory: Int, img: String)
case class UpdateProductForm(id: Int, name: String, price: Float, description: String, subCategory: Int, img: String)