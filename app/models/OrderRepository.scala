...
class OrderRepository @Inject() (dbConfigProvider: DatabaseConfigProvider,val shipmentRepository: ShipmentRepository,
  ...
  def create(date: String, payment: Int, shipment: Int, coupon: Int, user: Int): Future[Order] = db.run {
    (order.map(p => (p.date, p.payment, p.shipment, p.coupon, p.user))
  ...
  def getByUser(user: Int): Future[Seq[Order]] = db.run {
